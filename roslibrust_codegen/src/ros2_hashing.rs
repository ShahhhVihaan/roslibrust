...
fn get_field_type_id(field_type: &crate::FieldType) -> u8 {
    match field_type.array_info {
        Some(Some(_)) => {
            // Fixed length array
            let combined_name =
                FIELD_VALUE_TYPE_NAMES[field_type.field_type.as_str()].to_string() + "_ARRAY";
            FIELD_TYPE_NAME_TO_ID[&combined_name]
        }
        Some(None) => {
            // Unbounded array
            let combined_name = FIELD_VALUE_TYPE_NAMES[field_type.field_type.as_str()].to_string()
                + "_UNBOUNDED_SEQUENCE";
            FIELD_TYPE_NAME_TO_ID[&combined_name]
        }
        None => {
            // Not an array
            FIELD_TYPE_NAME_TO_ID[FIELD_VALUE_TYPE_NAMES[field_type.field_type.as_str()]]
        }
    }
}
